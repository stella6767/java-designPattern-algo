package javacore.basic.nio_socket;
...
public class Server {
    ...
    /**
     * https://codevang.tistory.com/185?category=827591
     * 논블로킹(non-blocking) 방식이란 데이터 송수신이 없을 때도 스레드가 블로킹 되지 않도록 하므로써 하나의 스레드에서 여러 입출력을 처리할 수 있도록 하는 방식
     * 멀티 스레드는 아님. 단지 빠르게 여러 채널에서 발생하는 유형별 이벤트를 돌아가면서 처리해주는 방식.  따라서 시간이 오래 걸리는 작업은 논블로킹 채널에서도 별도 스레드를 생성
     * @param args
     */
